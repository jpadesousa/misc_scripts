...
setwd("/Users/meyennf/Documents/Cambridge/Projects/PGCLC/2016Data/musBSSeq/Scatter/")

filenameRAW <- "musBSData_50CpG_5_1.txt" # to be used for HistoneValue imports (assumes positions are matched)
filenameRAW <- "musBSData_50CpG_5_1_Filter.txt" # to be used if positions are match indiviudally
...
testRead <- read.csv(filenameRAW,sep="\t",header=TRUE,skip=0,fill=TRUE,dec = ".",nrows = 5)
classes <- c("character","factor",rep("NULL",10),rep("numeric",ncol(testRead)-12))
HistValues <- read.csv(filenameRAW,sep="\t",header=TRUE,skip=0,fill=TRUE,dec = ".",colClasses = classes)

filenameRAW <- "HistoneValues_Mus_H3K9me3.txt"
testRead <- read.csv(filenameRAW,sep="\t",header=TRUE,skip=0,fill=TRUE,dec = ".",nrows = 5)
classes <- c(rep("NULL",12),rep("numeric",ncol(testRead)-12))
Input <- read.csv(filenameRAW,sep="\t",header=TRUE,skip=0,fill=TRUE,dec = ".",colClasses = classes)

HistValues <- cbind(HistValues,Input)

filenameRAW <- "IAPOverlap.txt"
testRead <- read.csv(filenameRAW,sep="\t",header=TRUE,skip=0,fill=TRUE,dec = ".",nrows = 5)
classes <- c("character","factor",rep("NULL",4),"factor",rep("NULL",ncol(testRead)-7))
Input <- read.csv(filenameRAW,sep="\t",header=TRUE,skip=0,fill=TRUE,dec = ".",colClasses = classes)
cbind(Input,IAP=rep(0,nrow(Input))) -> Input
Input[Input[,3] != "null",4] <- 1

HistValues <- cbind(HistValues,IAP=Input[,4])

remove(Input,testRead,classes,filenameRAW)

names(HistValues) <- c("Probe","Chromosome","H3K27ac","H3K4me3","H3K27me3","H3K9me2","panH3","H3K9me3","IAP")
...
MethValues$HighLowD4 <- MethValues$highD4 + 0.5 * MethValues$lowD4
MethValues$HighLowD6 <- MethValues$highD6 + 0.5 * MethValues$lowD6
...
#Scale Histone Values
M <- scale.histones(H,90,99)
#Perform splitting in pos and neg for Histone Values
M <- onoff.histones(H,T,70)
##################
...
plotName="musBSData_50CpG_5_1"
color.plot2C(N[pos,],M[pos,mark],names(M)[mark],cex=0.5,plotting="t",plotName=plotName)
density.hist(N[pos,],M[pos,mark],names(M)[mark],cex=cex,plotting="t",plotName=plotName)  #Label by Density of Mark

density.hist(N[pos,],M[pos,mark],names(M)[mark],cex=cex,plotting="p",plotName=plotName)  #Label by Density of Mark

color.hist(N[pos,],M[pos,mark],names(M)[mark],cex=cex,plotting="t",plotName=plotName)  #Label by Density of Mark
color.hist(N,M[,mark],names(M)[mark],cex=cex,plotting="t",plotName=plotName)  #Label by Density of Mark
...
M <- CG[as.integer(rownames(N)),]     #Overlay marks
