...
DataSetName <- "RawCounts.txt"

datasetInput <- read.delim(DataSetName,sep="\t",header=TRUE,skip=0,fill=TRUE,dec = ".",as.is=T)
datasetInputT <- datasetInput[,c(1,14:ncol(datasetInput))]

# Read in the full count table
just.raw.counts <- datasetInputT[,1+c(1:3,7:9)]

# Make up a condition table

condition <- factor(c(rep("ctrl", 3), rep("PCE", 3)))
