#####################################################################
# Hierachical Clustering of BS Data
#####################################################################

#####################################################################
# Transform data into matrix format, and preprocess
#####################################################################
...
#########################################################
# Clustering with hclust - Pearson distances and Ward distance func
#########################################################
...
pdf(paste(OutputName,"HierachicalClustering.pdf",sep=""),width=5,height=5,useDingbats=FALSE)

dend %>% set("leaves_pch", 16) %>% set("leaves_col", Color2Samples[Samples$sampleGroup]) %>% set("labels_cex", 0.7) %>%
  plot(ylab = "Height",main = "Distance:Pearson & Linkage: Ward.D",horiz = FALSE)

dev.off()

remove(dend,mat_data,distfunc,result)
