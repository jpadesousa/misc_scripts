...
ggplot(data=scores,aes(x=PC1, y=PC2)) +
  geom_point(pch=16,colour = Color2Samples[Samples$sampleGroup],size = 2) +
...
ggsave(paste(OutputName,"PCA.pdf",sep=""),width=5,height=5,useDingbats=FALSE)
...
remove(scores,pca.results,mat_data,loading,eigenvalues)
